...
struct gdt_ptr {
    uint64_t limit; // Tablonun toplam boyutu (Bayt cinsinden - 1). Bu yukardaki limitten farklı buradaki boyut tutuyor yukardaki ise ilk 16 biti.
    uint64_t  base;  // Tablonun başladığı tam adres
} __attribute__((packed));
