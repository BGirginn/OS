...
/* --- 64-BIT SAYFALAMA TABLOLARI İÇİN AYIRDIĞIMIZ ALAN --- */
.align 4096          /* İşlemcinin şart koştuğu 4KB hizalama çizgisi */

pml4_table:
.skip 4096           /* PML4 tablosu için 4KB yer ayır */
/*PML4 tablosunun içindeki her bir satır, veri depolamaz.
Bir alt seviyedeki PDPT (Page Directory Pointer Table) tablosunun RAM'deki fiziksel adresini ve o bellek alanına ait izinleri (yazılabilir mi, kullanıcı erişebilir mi vb.) tutar. */

pdpt_table:
.skip 4096           /* PDPT tablosu için 4KB yer ayır */

pd_table:
.skip 4096           /* PD tablosu için 4KB yer ayır */

pt_table:
.skip 4096           /* PT tablosu için 4KB yer ayır */
...
/* --- ADIM 2: TABLOLARI BİRBİRİNE BAĞLAMA --- */

/*bu pml4 lerin falan asıl amacı bir adres yani 0x0000000000201005 tarzındaki 64 bit adreslenen veriyi fiziksel nereye yazcağımızı bulmasıın sağlamak cpunun */

/* 1. PML4 -> PDPT Bağlantısı */
mov $pdpt_table, %eax     /* PDPT'nin RAM adresini EAX register'ına al */
or $0x3, %eax             /* Adresin sonuna Present(1) + Writable(2) izinlerini ekle (0x3) */
mov %eax, (pml4_table)    /* Bu izinli adresi PML4 tablosunun ilk satırına yaz */

/* 2. PDPT -> PD Bağlantısı */
mov $pd_table, %eax       /* PD'nin RAM adresini al */
or $0x3, %eax             /* İzinleri ekle */
mov %eax, (pdpt_table)    /* PDPT tablosunun ilk satırına yaz */

/* 3. PD -> PT Bağlantısı */
mov $pt_table, %eax       /* PT'nin RAM adresini al */
or $0x3, %eax             /* İzinleri ekle */
mov %eax, (pd_table)      /* PD tablosunun ilk satırına yaz */
